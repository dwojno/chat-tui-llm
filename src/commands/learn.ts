import { parseFileMentions, resolveMentionFile } from "../conversation/file-mentions";
import type { Command } from "./types";

const PREFIX = "/learn ";

/**
 * `/learn @path [@more ...]` — register source files for a future RAG corpus.
 * Paths are cwd-relative, validated on disk, and persisted in session state.
 */
export const learnCommand: Command = {
  ...
  run: async (input, { state, chat }) => {
    ...
    const added = state.addSources(valid);
