...
describe('runCommand', () => {
  it('turns a plain line into a streaming turn with the resolved temperature', async () => {
    ...
    expect(action).toMatchObject({ kind: 'turn', content: 'hello there', options: { temperature: 0.5, stream: true } })
  })
  ...
  it('/structured attaches the response schema', async () => {
    ...
    expect((action as { options: { structured_output: unknown } }).options.structured_output).toBe(ResponseSchema)
  })
