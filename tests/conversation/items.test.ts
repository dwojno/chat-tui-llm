...
describe('function-call helpers', () => {
  it('detects and extracts function calls in output', () => {
    const output = [functionCall('get_weather_data', { city: 'Paris' }), assistantMessage('hi')] as never
...
describe('toReplayInputItems', () => {
  it('rebuilds a function_call as a clean replayable item (drops parsed_arguments)', () => {
    const call = { ...functionCall('get_weather_data', { city: 'Paris' }, 'call_9'), parsed_arguments: { city: 'Paris' } }
...
describe('countUserTurns', () => {
  it('counts only user messages', () => {
    const items = [user('a'), toolOutput('r'), user('b'), { role: 'assistant', content: 'x' } as ResponseInputItem]
...
describe('renderItemsText', () => {
  it('flattens roles, function calls, and tool outputs into text', () => {
    ...
    expect(renderItemsText(items)).toBe(
      ['user: hello', 'assistant called get_weather_data({"city":"Paris"})', 'tool result: sunny'].join('\n'),
    )
  })

  it('extracts text from array-shaped content parts', () => {
    const item = { role: 'assistant', content: [{ type: 'output_text', text: 'part-a' }, { type: 'output_text', text: 'part-b' }] } as unknown as ResponseInputItem
