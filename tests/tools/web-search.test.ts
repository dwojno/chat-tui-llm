  ...
  it('formats results and strips HTML from snippets', async () => {
    mockFetch(() => ({
      ...
      json: async () => ({
        query: {
          search: [
            { title: 'Server-side rendering', snippet: 'renders <span class="hl">HTML</span> on the server' },
    ...
    expect(result).toBe(
      ['1. Server-side rendering: renders HTML on the server', '2. SSG: builds pages at build time'].join('\n'),
    )
  })
