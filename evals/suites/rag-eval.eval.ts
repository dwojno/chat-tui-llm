 ...
 */
const harness = createRagHarness({ suiteId: "rag", corpusDir: "evals/harness/rag-corpus" });
...
function toCase(ex: DatasetExample): { input: RagInput; expected: RagExpected } {
  ...
  return {
    input: { query: ex.question, category: ex.category, difficulty: ex.difficulty },
    expected: {
...
evalite<RagInput, RagResult, RagExpected>("rag pipeline", {
  ...
  columns: ({ input, output, expected }) => [
    { label: "Category", value: `${input.category ?? "?"} (${input.difficulty ?? "?"})` },
    { label: "Answer", value: output.answer },
    { label: "Gold", value: expected?.goldContextIds?.join(", ") || "(n/a)" },
    { label: "Retrieved", value: output.retrievedSources.join(", ") || "(none)" },
