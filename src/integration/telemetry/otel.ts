...
import { configureTelemetry } from "../../agent/telemetry";
...
export function startTelemetry(config: TelemetryConfig): void {
  ...
  sdk = new NodeSDK({
    ...
    traceExporter: new OTLPTraceExporter({ url: `${base}/v1/traces`, headers: config.headers }),
    metricReader: new PeriodicExportingMetricReader({
      exporter: new OTLPMetricExporter({ url: `${base}/v1/metrics`, headers: config.headers }),
    }),
  });
  sdk.start();
}
